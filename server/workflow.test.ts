...
describe('research workflow (e2e)', () => {
  ...
  it('create query → run research → get result → submit feedback → metrics', async () => {
    const query = await insertResearchQuery({ query_text: 'What is agentic AI?' });
    expect(query.id).toBeGreaterThan(0);
    expect(await getResearchQuery(query.id)).not.toBeNull();

    const outcome = await runResearch(query.id);
    ...
    const result = await getResearchResult(outcome.researchResultId);
    ...
    const resultsByQuery = await listResearchResultsByQueryId(query.id);
    expect(resultsByQuery.length).toBe(1);
    expect(resultsByQuery[0].id).toBe(outcome.researchResultId);

    const citations = await listCitationsByResultId(outcome.researchResultId);
    ...
    const feedbackList = await listUserFeedbackByResultId(outcome.researchResultId);
  ...
  it('workflow with vault documents included in retrieval', async () => {
    ...
    const query = await insertResearchQuery({ query_text: 'Explain agentic systems' });
    const outcome = await runResearch(query.id);
    expect(outcome.researchResultId).toBeGreaterThan(0);
  ...
  it('run with invalid query id fails and updates status', async () => {
    await expect(runResearch(99999)).rejects.toThrow(/Query not found|not found/i);
    const query = await getResearchQuery(99999);
